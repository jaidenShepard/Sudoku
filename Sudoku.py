...
from cell import Cell, Membership
...
class Sudoku:
    ...
    def create_raw_board(self) -> List[Cell]:
        ...
        for i in range(0, 10):
            ...
            for current_node in node_buff:
                for other_node in node_buff:
                    if current_node != other_node:
                        current_node.add_memember_to(Membership.ROW, other_node)
            ...
            for current_node in node_buff:
                for other_node in node_buff:
                    if current_node != other_node:
                        current_node.add_memember_to(Membership.COLUMN, other_node)
            ...
            for current_node in node_buff:
                for other_node in node_buff:
                    if current_node != other_node:
                        current_node.add_memember_to(Membership.GROUP, other_node)
    ...
    def get_grid(self, difficulty: str) -> List[int]:
        """
        ...
        for i, cell in enumerate(self.cells):
            if i in given_cells:
                game_grid.append(cell.value or 0)
            else:
                game_grid.append(0)
